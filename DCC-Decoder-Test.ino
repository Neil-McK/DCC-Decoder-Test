...
Modified by N McKechnie 14/02/2018
...
Amended NMcK:
  Allow different current limits on main and programming tracks.
...
// NOTE REGISTER LIST ELEMENTS ARE DECLARED WITH "VOLATILE" QUALIFIER TO ENSURE THEY ARE PROPERLY UPDATED BY INTERRUPT ROUTINES

RegisterList mainRegs(MAX_MAIN_REGISTERS);    // create list of registers for MAX_MAIN_REGISTER Main Track Packets
RegisterList progRegs(2);                     // create a shorter list of only two registers for Program Track Packets

CurrentMonitor mainMonitor(CURRENT_MONITOR_PIN_MAIN,"<p2>", CURRENT_SAMPLE_MAX_MAIN);  // create monitor for current on Main Track
CurrentMonitor progMonitor(CURRENT_MONITOR_PIN_PROG,"<p3>", CURRENT_SAMPLE_MAX_PROG);  // create monitor for current on Program Track

unsigned int oneBitTotalDurationTimer;
unsigned int oneBitPulseDurationTimer;
unsigned int zeroBitTotalDurationTimer;
unsigned int zeroBitPulseDurationTimer;
...
void setup(){  
  ...
  Serial.print(F("<iDCC++ BASE STATION FOR ARDUINO "));      // Print Status to Serial Line regardless of COMM_TYPE setting so use can open Serial Monitor and check configurtion 
...
/* NMCK ADDED START ==================================*/
  OCR1A=oneBitTotalDurationTimer;
  OCR1B=oneBitPulseDurationTimer;
/* NMCK ADDED END ==================================*/
  ...
  // CONFIGURE TIMER_3 TO OUTPUT 50% DUTY CYCLE DCC SIGNALS ON OC3B INTERRUPT PINS
...
#define DCC_SIGNAL(R,N) \
  ...
  if(R.currentReg->activePacket->buf[R.currentBit/8] & R.bitMask[R.currentBit%8]){     /* IF bit is a ONE */ \
                OCR ## N ## A=oneBitTotalDurationTimer;                                /*   set OCRA for timer N to full cycle duration of DCC ONE bit NMCK */ \
                OCR ## N ## B=oneBitPulseDurationTimer;                                /*   set OCRB for timer N to half cycle duration of DCC ONE bit NMCK*/ \
  } else{                                                                              /* ELSE it is a ZERO */ \
                OCR ## N ## A=zeroBitTotalDurationTimer;                               /*   set OCRA for timer N to full cycle duration of DCC ZERO bit NMCK*/ \
                OCR ## N ## B=zeroBitPulseDurationTimer;                               /*   set OCRB for timer N to half cycle duration of DCC ZERO bit NMCK*/ \
  }                                                                                    /* END-ELSE */ \ 
                                                                                       \ 
...
/* NMCK ADDED START ==================================*/
void SetPulseWidth(byte bitValue, unsigned int bitHalfLength) {
  if (bitValue == 0) {
    zeroBitTotalDurationTimer = 16 * bitHalfLength * 2 - 1;
    zeroBitPulseDurationTimer = 16 * bitHalfLength - 1;
  } else {
    oneBitTotalDurationTimer = 16 * bitHalfLength * 2 - 1;
    oneBitPulseDurationTimer = 16 * bitHalfLength - 1;
  }
}

void ParsePulseCommand(char *c) {
  int bitValue, bitHalfLength;
  if (sscanf(c, "%d %d", &bitValue, &bitHalfLength) == 2 &&
