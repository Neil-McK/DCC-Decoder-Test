...
void RegisterList::loadPacket(int nReg, byte *b, int nBytes, int nRepeat, int printFlag) volatile {
  ...
  while(nextReg!=NULL) {              // pause while there is a Register already waiting to be updated -- nextReg will be reset to NULL by interrupt when prior Register updated fully processed
    if (nReg == 0)
      checkForAck();                  // check for current increase in case command in progress  
  }
...
void RegisterList::prepareForAck() volatile{
  // determine current sense value
  long total = 0;
  for (int i=0; i<ACK_BASE_COUNT; i++)
    total += analogRead(CURRENT_MONITOR_PIN_PROG);
  baseCurrent = total / ACK_BASE_COUNT;
  ackReceived = 0;
}

///////////////////////////////////////////////////////////////////////////////

void RegisterList::checkForAck() volatile {
  int currentCurrent = analogRead(CURRENT_MONITOR_PIN_PROG);
  if(currentCurrent - baseCurrent > ACK_SAMPLE_THRESHOLD) {
    ackReceived = 1;
  }
}
...
void RegisterList::readCV(char *s) volatile{
  ...
  for(int i=0;i<8;i++){
    // Read one bit on each loop, by verifying whether bit value is 1 (ack means yes).
    prepareForAck(); 
    ...
    bitWrite(bValue,i,ackReceived);
  }

  // set up to re-verify entire byte
  prepareForAck(); 

  bRead[0]=0x74+(highByte(cv)&0x03);   
  bRead[2]=bValue;  
